...
def process_adverts(data):
    return [
        {
            'url': f"https://jiji.co.tz{i.get('url')}",
            'propertyId': i.get('id'),
...
def fetch_data(slug):
    all_data = []
    url = f"https://jiji.co.tz/api_web/v1/listing?slug={slug}"
...
with ThreadPoolExecutor(max_workers=threads) as executor:
    ...
    s3.put_object(Body=log_stringio.getvalue(), Bucket=bucket_name, Key="logs/jiji_co_tz/url-extractor-logs.txt")
