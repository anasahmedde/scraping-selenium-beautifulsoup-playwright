...
log = logging.getLogger("property-pro-co-ke-url-extractor")
...
def extract_links(url):
    ...
    if soup.find(string='Sorry we could not find any property matching your criteria.', name='p') or soup.find(string='Oops, an error occurred', name='h1'):
        return []
    log.info(url)
    return [[link, ids, listingType, price] for link, ids, listingType, price in zip(
            ['https://propertypro.co.ke'+a.parent.get('href') for a in soup.select('h2.listings-property-title')],
...
urls = [f"https://propertypro.co.ke/property-for-sale?page={i}" for i in range(140)]
urls += [f"https://propertypro.co.ke/property-for-rent?page={i}" for i in range(500)]
urls += [f"https://propertypro.co.ke/property-for-shortlet?page={i}" for i in range(4)]
...
if __name__ == '__main__':
    ...
    sendData(links, ['url', 'propertyId', 'listingType', 'price'], databaseName, 'propertyURLs')
    log.info("URL extraction completed successfully.")
    s3 = boto3.client("s3", region_name=aws_region_name)
    s3.put_object(Body=log_stringio.getvalue(), Bucket=bucket_name, Key="logs/property-pro-co-ke/url-extractor-logs.txt")  
