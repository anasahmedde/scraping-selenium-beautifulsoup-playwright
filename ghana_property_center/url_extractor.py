...
log = logging.getLogger("ghanaPropertyCentre-url-extractor")
...
def extract_links(url):
    ...
    return [[link, ids, price] for link, ids, price in zip(
            ['https://ghanapropertycentre.com'+a.parent.get('href') for a in soup.select('div.row.property-list h4.content-title')],
...
urls = [f"https://ghanapropertycentre.com/for-rent/short-let?page={i}" for i in range(1, 25)]
urls += [f"https://ghanapropertycentre.com/for-rent?page={i}" for i in range(1, 130)]
urls += [f"https://ghanapropertycentre.com/for-sale?page={i}" for i in range(1, 300)]
...
if __name__ == '__main__':
    ...
    s3.put_object(Body=log_stringio.getvalue(), Bucket=bucket_name, Key="logs/ghanaPropertyCentre/url-extractor-logs.txt")
