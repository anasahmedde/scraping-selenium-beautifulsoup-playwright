...
log = logging.getLogger("jiji-com-et-url-extractor")
...
def process_adverts(data):
    return [
        {
            'url': f"https://jiji.com.et{i.get('url')}",
            'propertyId': i.get('id'),
...
def fetch_data(slug):
    all_data = []
    url = f"https://jiji.com.et/api_web/v1/listing?slug={slug}"
...
with ThreadPoolExecutor(max_workers=threads) as executor:
    ...
    s3.put_object(Body=log_stringio.getvalue(), Bucket=bucket_name, Key="logs/jiji_com_et/url-extractor-logs.txt")
