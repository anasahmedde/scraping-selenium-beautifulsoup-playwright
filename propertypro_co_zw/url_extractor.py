...
log = logging.getLogger("property-pro-co-zw-url-extractor")
...
def extract_links(url):
    ...
    return [[link, ids, listingType, price, currency] for link, ids, listingType, price, currency in zip(
        ['https://propertyprozw.com'+a.parent.get('href') for a in soup.select('h2.listings-property-title')],
        ...
        ["USD" for i in range(len(soup.select('h2.listings-property-title')))]
    )]


urls = [f"https://propertyprozw.com/property-for-sale?page={i}" for i in range(45)]
urls += [f"https://propertyprozw.com/property-for-rent?page={i}" for i in range(20)]
urls += [f"https://propertyprozw.com/property-for-shortlet?page={i}" for i in range(1)]
...
if __name__ == '__main__':
    ...
    s3.put_object(Body=log_stringio.getvalue(), Bucket=bucket_name, Key="logs/property-pro-co-zw/url-extractor-logs.txt")  
