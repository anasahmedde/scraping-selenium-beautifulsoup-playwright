...
def fetch_all_city_data(df, max_workers=8):
    """
...
def send_data_and_checkpoint(df_chunk, log, db):
    """
    For each country in this chunk, send its rows to the
    <country>.collection (lower-cased), then mark them processed.
    """
    # send per country
    for country, group in df_chunk.groupby('consolidatedCountry'):
        coll_name = country.lower()
        log.info(f"Sending {len(group)} records to collection '{coll_name}'")
        send_data(group, finalDatabaseName, coll_name, log)

    # then mark all rows in df_chunk as processed
    mark_records_processed(df_chunk, db)
...
def main():
    try:
        ...
        filters = [
            seso_filter,
            buyrentkenya_filter,
            kenyaPropertyCentre_filter,
            propertypro_co_ke_filter,
            jiji_co_ke_filter,
            lamudi_filter
        ]
        ...
        chunk_size = 400
        batch_size = 50
        num_threads = 8  # Set threads as required
        ...
        for idx, df in enumerate(filter_results, start=1):
            ...
            city_results = fetch_all_city_data(df, max_workers=8)
