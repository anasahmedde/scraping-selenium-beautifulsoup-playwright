...
parser = argparse.ArgumentParser(description='Run Docker containers for specific folders and monitor their activity.')
parser.add_argument('-f', '--folders', nargs='+', help='List of folder names to process', default=["ethiopian_properties", "global_remax", "house_in_rwanda", "lamudi", "mubawab", "property24", "property24_co_ke", "propertypro_co_ke", "propertypro_co_ug", "propertypro_co_zw", "propertypro_ng", "prophuntgh", "real_estate_tanzania", "seso", "knightfrank_ug", "jiji_co_ke", "jiji_ug", "jiji_co_tz", "jiji_com_gh", "jiji_ng", "jiji_com_et"])
...
def monitor_container(container_name, script_name, folder_name):
    ...
    initial_check_interval = 1 * 60  # 1 minutes
    inactivity_check_interval = 1 * 100  # 1 minutes
    ...
    while True:
        ...
        if current_log_length > last_log_length:
            ...
            if not grace_period_passed and time.time() - start_time > initial_check_interval:
                # Grace period just passed, start checking for inactivity from now.
                grace_period_passed = True
                last_log_length = current_log_length  # Update to avoid immediate inactivity notice.
        ...
        # Adjust sleep interval based on whether the grace period has passed.
...
def run_script_in_container(folder_name, script_name):
    ...
    # Start a thread to monitor the container
    #monitor_thread = Thread(target=monitor_container, args=(container_name, script_name, folder_name))
    #monitor_thread.start()
    container.wait()  # Wait for the container to finish running
    #monitor_thread.join()  # Ensure the monitoring thread has completed
...
# Main execution
# folders = ["global_remax"]
args = parser.parse_args()
folders = args.folders

# python your_script_name.py --folders folder1 folder2 ..

scripts = ["url_extractor.py", "detail_extractor.py"]
#scripts = ["url_extractor.py"]
...
topic_arn = "arn:aws:sns:us-east-2:561513845508:rehani-scraping-sns"  # Your SNS topic ARN
